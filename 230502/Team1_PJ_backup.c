            ...
            while(1) // 1반복문 1움직임(1턴)
            {
                ...
                if(move=='w') // 위로 이동
                {
                    if(field[i-1][j]==0) // 이동하려는 칸이 빈칸일 때
                    {
                        field[i][j]=0;
                        field[i-1][j]=1;
                    }else if(field[i-1][j]==2){ // 이동하려는 칸이 먹이일 때
                        field[i][j]=0;
                        field[i-1][j]=1;
                        food++;
                    }else if(field[i-1][j]==3){ // 이동하려는 칸이 아이템일 때
                        field[i][j]=0;
                        field[i-1][j]=1;
                    ...
                    }else if(field[i-1][j]==4){ // 이동하려는 칸이 벽일 때
                        continue;
                    }else if(field[i-1][j]==5 || field[i-1][j]==7){ // 이동하려는 칸이 적일 때
                        field[i][j]=0;
                        game_over=1;
                        continue;
                    }else if(field[i-1][j]==6){ // 이동하려는 칸이 처치 가능한 적일 때
                        field[i][j]=0;
                        field[i-1][j]=1;
                    }else if(field[i-1][j]==8){ // 이동하려는 칸이 먹이와 겹친 처치 가능한 적일 때
                        field[i][j]=0;
                        field[i-1][j]=1;
                        food++;
                    } // 이하 복붙하고 이동 방향만 변경
                }else if(move=='s'){ // 아래로 이동
                    if(field[i+1][j]==0)
                    {
                        field[i][j]=0;
                        field[i+1][j]=1;
                ...
                while(1) // 벽에 막혀 이동 불가일 때 돌아오기 위한 반복문
                {
                    ...
                    if(move_enemy==0) // 적 위로 이동
                    {
                        ...
                        }else if(field[i-1][j]==2){ // 먹이와 충돌 시
                            if(field[i][j]==5)
                            {
                                field[i][j]=0;
                                field[i-1][j]=7;
                        ...
                        }else if(field[i-1][j]==3){ // 아이템과 충돌 시
                            if(field[i][j]==5)
                            {
                                field[i][j]=0;
                                field[i-1][j]=5;
                        ...
                        }else if(field[i-1][j]>=4 && field[i-1][j]<=8){ // 벽과 충돌 또는 적끼리 충돌 시
                            continue;
                        }else if(field[i-1][j]==0){ // 빈공간으로 이동 시
                            if(field[i][j]==5)
                            {
                                field[i][j]=0;
                                field[i-1][j]=5;
